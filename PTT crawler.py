...
def parseGos(link, g_id, acd):
    ...
    d = {"a_ID": g_id, "acd": acd, "b_作者": author, "c_標題": title, "d_日期": date,
...
if __name__ == "__main__":
    PttName, ParsingPage = 'BuyTogether', int(3999)
    start_time = time.time()
    print('Start parsing ' + PttName + '....')
    fileName = 'D:\PCB\data-' + PttName + '-' + datetime.now().strftime('%Y%m%d%H%M%S') + '.json'
    # 檢查看板是否為18禁,有些看板為18禁
    soup = over18(PttName)
